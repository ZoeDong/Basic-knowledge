...
# 4.17-4.18
## 为什么虚函数效率低？
存在动态跳转，这会导致**分支预测失败**，流水线排空。 
- 如果不是虚函数，那么在编译时期，其相对地址是确定的，编译器可以直接生成jmp/invoke指令；
- 如果是虚函数，多出来的一次查找vtable所带来的开销，倒是次要的；关键在于，这个函数地址是动态的，譬如取到的地址在eax里，则在call eax之后的那些已经被预取进入流水线的所有指令都将失效。流水线越长，一次分支预测失败的代价也就越大。

## new / delete 与 malloc / free
- 都可用于内存的动态申请和释放
### 不同点：
- malloc/free 只进行分配内存/释放内存的工作；new/delete 进行分配内存/释放内存的工作 + 调用构造/析构函数
  - 调用构造/析构函数：
    1. 可以对变量进行**初始化**
    2. **指针变量/非内部数据类型 必须显式分配释放内存**，光用maloc/free无法满足动态对象的要求。例如：对象当中有一个指针变量p，在构造函数中为它动态申请内存；在析构函数中，应该先判断该指针是否为空，如果不为空，则使用delete释放内存（释放p指向的空间），然后再把该指针设置为NULL（断开p和指向空间的链接）。
    3. 构造函数里面申请的空间，不是必须在析构函数里面释放，只要在不使用的时候释放就可以，释放后把变量设置为NULL，那怕析构的时候，再次delete，也是delete NULL ，不会产生问题；同样的，new也不是只可以在构造函数时候调用，构造时候初始化指针为NULL，其他时刻来new 也无妨。
- new是保留字，C++运算符，**不需要头文件支持**，支持**重载**；malloc是C/C++语言标准库函数需要**头文件库函数支持**. 使用malloc/free需要包含 #include<cstdlib> 或<stdlib>，支持**覆盖**
- new自动计算要分配的空间大小，malloc需要手工计算
- malloc和free**返回**的是**void类型指针**（必须进行类型转换），不是类型安全的；new和delete返回的是**具体类型指针**，是类型安全的。
...
### 访问delete后的空间
delete后你只是放弃了对这片内存的所有权，获得所有权的人对这片内存做什么（或者说什么都不做）都不关你的事。访问 delete 之后的内存是一个未定义行为。未定义行为可能产生任何结果，包括但不限于：产生期望的结果，产生未期望的结果，产生随机的结果，产生无法解释的结果，运行错误，随机的运行时错误，编译错误等等。因此delete之后需要把指针置为NULL。如果不清空指针，继续访问就会出现未定义的结果。
...
int main() {
    //无法调用构造/析构函数 arr没有成功初始化
    test* tt1= (test*)malloc(sizeof(test));
    //...
    free(tt1);
