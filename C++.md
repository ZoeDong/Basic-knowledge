...
## 内存溢出与内存泄露
  ...
  - 内存泄露：无法释放已申请的内存；
    - 内存泄露的原因：new创建的对象没有及时delete/delete错误（使用已经释放的对象/同一块内存释放两次）：
      ...
      2. delete掉一个void*类型的指针：
      导致**没有调用到对象的析构函数**，析构的所有清理工作都没有去执行
      1. 释放对象数组时只调用了delete而非delete []：
      导致只有对象数组的第一个对象的析构函数得到执行并回收了内存占用，数组的其他对象所占内存得不到回收
      1. 对指针/对象进行浅拷贝：
      导致拷贝前后两个对象指向同一个地址空间，内存被释放两次；【深浅拷贝】
      1. 没有正确地清除嵌套的对象指针
      ...
      1. 没有将基类的析构函数定义为虚函数:
      当**基类指针**指向派生类对象时，派生类中有动态分配的内存，如果基类的析构函数不是virtual，那么**子类的析构函数将不会被调用**，子类的资源没有正确释放
        ```C++
        ...
        ```
      1. 在外部函数分配的堆内存，不要在调用函数里面释放，而在外部函数内释放；
    - 内存泄漏避免方法：
